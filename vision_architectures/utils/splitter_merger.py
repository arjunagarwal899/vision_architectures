...
# %% auto 0
__all__ = ['SplitterConfig', 'Splitter']
...
class Splitter:
    ...
    def expand(self, x: torch.Tensor) -> torch.Tensor:
        ...
        start_dim = x.ndim - self.config.split_dims
        end_dim = x.ndim
        for i, dim in zip(range(self.config.split_dims), range(start_dim, end_dim)):
            expansion = expanded_shape[i] - x.shape[dim]
            if expansion > 0:
                if self.config.extend_mode == "pad":
                    padding = (0, expansion) + (0, 0) * (self.config.split_dims - i - 1)
                    x = F.pad(x, padding)
